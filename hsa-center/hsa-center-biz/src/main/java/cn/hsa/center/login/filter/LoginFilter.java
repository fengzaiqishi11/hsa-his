package cn.hsa.center.login.filter;
...
public class LoginFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        excludeList.add("/center/login/authCode");
        excludeList.add("/center/login/doLogin");
    }
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (isExclude(request)) {
            ...
            SysUserDTO userDto = (SysUserDTO) session.getAttribute("CENTER_SESSION_USER_INFO");
            // 判断session中是否有用户数据，如果有，则返回true，继续向下执行
            if (userDto != null) {
                filterChain.doFilter(servletRequest, servletResponse);
            } else {
                // {"code":99,"message":"未登录或接口访问超时，请重新登录！","type":"error"}
                WrapperResponse wr = WrapperResponse.error(99, "未登录或接口访问超时，请重新登录！", null);
