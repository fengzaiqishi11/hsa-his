...
//            data = inptMrisInfoDAO.queryData(sql_str);
    ...
    @Override
    public List<LinkedHashMap<String, Object>> exportTcmMrisInfoForHqms(Map map) throws Exception {
        List<LinkedHashMap<String, Object>> data =inptMrisInfoDAO.getTcmMrisBaseInfoForHqms(map);
        SysParameterDTO sysParameterDTO =null;
        Map<String, Object> isInsureUnifiedMap = new HashMap<>();
        isInsureUnifiedMap.put("hospCode", map.get("hospCode"));
        isInsureUnifiedMap.put("code", "BASFBM");
        ...
        if (data!=null&&data.size()>0) {
            for (LinkedHashMap<String, Object> ma : data) {
                ...
                if (!MapUtils.isEmpty(sfMaps)) {
                    String key = (String) ma.get("A23C");
                    if (StringUtils.isNotEmpty(key)) {
                        ma.put("A23C", sfMaps.get(key)!=null?sfMaps.get(key):"19");
                    }
                }else {
                    ma.put("A23C", "19");
                }
                ...
                if (mrisDiagnose != null && mrisDiagnose.size() > 0) {
                    for (Map<String, Object> mrisDiagnosedata : mrisDiagnose) {
                        if ((mrisDiagnosedata.get("disease_code") != null && mrisDiagnosedata.get("disease_code").equals("1"))) {
                            diagnoseMap.put("C03C", changeData(mrisDiagnosedata.get("disease_icd10")));
                            diagnoseMap.put("C04N", changeData(mrisDiagnosedata.get("disease_icd10_name")));
                            diagnoseMap.put("C05C", changeData(mrisDiagnosedata.get("in_situation_code")));
                        } else {
                            diagnoseMap.put("C06x0" + i + "C", changeData(mrisDiagnosedata.get("disease_icd10")));
                            diagnoseMap.put("C07x0" + i + "N", changeData(mrisDiagnosedata.get("disease_icd10_name")));
                            diagnoseMap.put("C08x0" + i + "C", changeData(mrisDiagnosedata.get("in_situation_code")));
                            i = i + 1;
                        }
                    }
                }
                ma.putAll(diagnoseMap);
                List<Map<String, Object>> mrisOperInfo = inptMrisInfoDAO.getTcmMrisOperInfoForHqms(diagnoseParam);
                LinkedHashMap<String, Object> operInfoMap = new LinkedHashMap<>();
                if (mrisOperInfo != null && mrisOperInfo.size() > 0) {
                    Map<String, Object> dataMap = mrisOperInfo.get(0);
                    operInfoMap.put("C14x01C", changeData(dataMap.get("oper_disease_icd9")));
