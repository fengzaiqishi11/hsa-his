...
public class PasswordCheckUtil {
     ...
     *  密码复杂度校验正则表达式
     * **/
    private static final String PASSWORD_CHECK_STRING=
            "^^(?![a-zA-z]+$)(?!\\d+$)(?![!@#$%^&*_-]+$)(?![a-zA-z\\d]+$)(?![a-zA-z!@#$%^&*_-]+$)(?![\\d!@#$%^&*_-]+$)[a-zA-Z\\d!@#$%^&*_-]+$";

    private static final Pattern PASSWORD_CHECK_PATTERN = Pattern.compile(PASSWORD_CHECK_STRING);

    public static void main(String[] args) {
      //System.out.println( pwdComplexityVerify("888888",3,true));
      //System.out.println( pwdComplexityVerify("poiuyt",3,true));
      //System.out.println( pwdComplexityVerify("1qazw",3,true));
      //System.out.println( pwdComplexityVerify("4eszaz",3,true));
      //System.out.println( pwdComplexityVerify("4rfvas",3,true));
      System.out.println("poi1a- "+pwdComplexityVerify("poi1a-"));
      System.out.println("psi1a- "+pwdComplexityVerify("pwi1a-"));
      System.out.println("psaaaa- "+pwdComplexityVerify("psaaaa-"));
      System.out.println("psaa- "+pwdComplexityVerify("psaa-"));

    }
    public static String pwdComplexityVerify(String password){
      boolean isPasswordShort = checkPasswordLength(password,"6","16");
      if(!isPasswordShort){
          return "密码长度不得低于6位,最长不超过16位!";
      }
      boolean containsLettersDigitsCharacters =  Pattern.matches(PASSWORD_CHECK_STRING,password);

      if(!containsLettersDigitsCharacters){
          return "密码必须同时包含字母数字与特殊字符!";
      }
      return pwdComplexityVerify(password,3,true);
    }

    public static String pwdComplexityVerify(String password, int repetitions, boolean isLower) {
        boolean isContainsSameChar =  checkSequentialSameChars(password, repetitions);
        if(isContainsSameChar){
            return "密码不能包含超过"+repetitions +" 个相同的字符";
        }
        boolean isContinuousCharacter = checkSequentialChars(password, repetitions, false);
        if(isContinuousCharacter){
            return "密码字段不能包含超过"+repetitions +" 个连续的字符(不区分大小写)";
        }
        boolean isLateralKeyboardSite = checkLateralKeyboardSite(password, repetitions, false);
        if(isLateralKeyboardSite){
            return "密码不能包含键盘上横向的超过"+repetitions+" 个连续字符(不区分大小写)";
        }
        return "";
    }
    ...
    public static boolean checkPasswordLength(String password, String minNum, String maxNum) {
        boolean flag =false;
        if (StrUtil.isEmpty(maxNum))  {
            minNum = StrUtil.isEmpty(minNum) == true ? "0":minNum;
